...
[📄 View Source Code](https://github.com/Ripe-Foundation/ripe-protocol/blob/5c30234e855cd8cbb54d199aef48e5ee07538244/contracts/data/Ledger.vy)

## Purpose

`Ledger` is the protocol's authoritative shared state store. It records
user-to-vault participation, debt and interest state, reward checkpoints,
auction membership, Human Resources data, bond epochs, and Endaoment debt. It
is not a general-purpose user entry point: mutators are restricted to the
department responsible for each state domain.
...
The constructor also requires a Defaults-address argument whose value may be
zero. When it is nonzero,
deployment calls `ripeAvailForRewards()`, `ripeAvailForHr()`, and
`ripeAvailForBonds()` and copies those three initial allocations into Ledger;
any failed typed call reverts deployment. A zero Defaults address leaves all
three values at zero. These are copied seeds, not live references to later
Defaults values.
...
- **Vault participation:** each user's list of vault IDs. Vault-local supported
  assets, user asset enumeration, and balances belong to each vault's
  `VaultData` state rather than Ledger.
...
- User-to-vault participation lists are maintained alongside state changes so
  downstream modules can find the vaults that may hold a user's positions;
  each vault remains authoritative for its assets and balances.
...
| --- | --- | --- |
| `checkAndUpdateLastTouch(address _user, bool _shouldCheck, address _mc)` | `2–3` | `_mc = empty(address)` |
...
| `assetDepositPoints(uint256 arg0, address arg1)` | `view` | `(uint256 balancePoints, uint256 lastBalance, uint256 lastUsdValue, uint256 ripeStakerPoints, uint256 ripeVotePoints, uint256 ripeGenPoints, uint256 lastUpdate, uint256 precision)` | — |
| `badDebt()` | `view` | `uint256` | — |
| `borrowIntervals(address arg0)` | `view` | `(uint256 start, uint256 amount)` | — |
...
| `createNewFungibleAuction((address,uint256,address,uint256,uint256,uint256,uint256,bool) _auc)` | `nonpayable` | `uint256` | `uint256` |
...
| `flushUnrealizedYield()` | `nonpayable` | `uint256` | `uint256` |
| `fungLiqUsers(uint256 arg0)` | `view` | `address` | — |
| `fungibleAuctionIndex(address arg0, uint256 arg1, address arg2)` | `view` | `uint256` | — |
| `fungibleAuctions(address arg0, uint256 arg1)` | `view` | `(address liqUser, uint256 vaultId, address asset, uint256 startDiscount, uint256 maxDiscount, uint256 startBlock, uint256 endBlock, bool isActive)` | — |
...
| `getArbActionBlock()` | `view` | `uint256` | `uint256` |
| `getBorrowDataBundle(address _user)` | `view` | `((uint256 amount, uint256 principal, (uint256 ltv, uint256 redemptionThreshold, uint256 liqThreshold, uint256 liqFee, uint256 borrowRate, uint256 daowry) debtTerms, uint256 lastTimestamp, bool inLiquidation) userDebt, (uint256 start, uint256 amount) userBorrowInterval, bool isUserBorrower, uint256 numUserVaults, uint256 totalDebt, uint256 numBorrowers)` | `BorrowDataBundle` |
| `getBorrowPointsBundle(address _user)` | `view` | `((uint256 lastPrincipal, uint256 points, uint256 lastUpdate) userPoints, (uint256 lastPrincipal, uint256 points, uint256 lastUpdate) globalPoints, uint256 userDebtPrincipal)` | `BorrowPointsBundle` |
| `getDepositLedgerData(address _user, uint256 _vaultId)` | `view` | `(bool isParticipatingInVault, uint256 numUserVaults)` | `DepositLedgerData` |
| `getDepositPointsBundle(address _user, uint256 _vaultId, address _asset)` | `view` | `((uint256 balancePoints, uint256 lastBalance, uint256 lastUpdate) userPoints, (uint256 balancePoints, uint256 lastBalance, uint256 lastUsdValue, uint256 ripeStakerPoints, uint256 ripeVotePoints, uint256 ripeGenPoints, uint256 lastUpdate, uint256 precision) assetPoints, (uint256 lastUsdValue, uint256 ripeStakerPoints, uint256 ripeVotePoints, uint256 ripeGenPoints, uint256 lastUpdate) globalPoints)` | `DepositPointsBundle` |
| `getEpochData()` | `view` | `(uint256, uint256)` | `(uint256, uint256)` |
| `getFungibleAuction(address _liqUser, uint256 _vaultId, address _asset)` | `view` | `(address liqUser, uint256 vaultId, address asset, uint256 startDiscount, uint256 maxDiscount, uint256 startBlock, uint256 endBlock, bool isActive)` | `FungibleAuction` |
| `getFungibleAuctionDuringPurchase(address _liqUser, uint256 _vaultId, address _asset)` | `view` | `(address liqUser, uint256 vaultId, address asset, uint256 startDiscount, uint256 maxDiscount, uint256 startBlock, uint256 endBlock, bool isActive)` | `FungibleAuction` |
| `getNumBorrowers()` | `view` | `uint256` | `uint256` |
| `getNumUserVaults(address _user)` | `view` | `uint256` | `uint256` |
| `getRepayDataBundle(address _user)` | `view` | `((uint256 amount, uint256 principal, (uint256 ltv, uint256 redemptionThreshold, uint256 liqThreshold, uint256 liqFee, uint256 borrowRate, uint256 daowry) debtTerms, uint256 lastTimestamp, bool inLiquidation) userDebt, uint256 numUserVaults)` | `RepayDataBundle` |
| `getRipeBondData()` | `view` | `(uint256 paymentAmountAvailInEpoch, uint256 ripeAvailForBonds, uint256 badDebt)` | `RipeBondData` |
| `getRipeHq()` | `view` | `address` | — |
| `getRipeRewardsBundle()` | `view` | `((uint256 borrowers, uint256 stakers, uint256 voters, uint256 genDepositors, uint256 newRipeRewards, uint256 lastUpdate) ripeRewards, uint256 ripeAvailForRewards)` | `RipeRewardsBundle` |
| `globalBorrowPoints()` | `view` | `(uint256 lastPrincipal, uint256 points, uint256 lastUpdate)` | — |
| `globalDepositPoints()` | `view` | `(uint256 lastUsdValue, uint256 ripeStakerPoints, uint256 ripeVotePoints, uint256 ripeGenPoints, uint256 lastUpdate)` | — |
| `greenPoolDebt(address arg0)` | `view` | `uint256` | — |
| `hasDebt(address _user)` | `view` | `bool` | `bool` |
| `hasFungibleAuction(address _liqUser, uint256 _vaultId, address _asset)` | `view` | `bool` | `bool` |
| `hasFungibleAuctions(address _liqUser)` | `view` | `bool` | `bool` |
...
| `isBorrower(address _user)` | `view` | `bool` | `bool` |
| `isHrContributor(address _contributor)` | `view` | `bool` | `bool` |
| `isLockedAccount(address arg0)` | `view` | `bool` | — |
| `isParticipatingInVault(address _user, uint256 _vaultId)` | `view` | `bool` | `bool` |
| `isPaused()` | `view` | `bool` | — |
| `isUserInLiquidation(address _user)` | `view` | `bool` | `bool` |
...
| `ripeRewards()` | `view` | `(uint256 borrowers, uint256 stakers, uint256 voters, uint256 genDepositors, uint256 newRipeRewards, uint256 lastUpdate)` | — |
...
| `setFungibleAuction(address _liqUser, uint256 _vaultId, address _asset, (address,uint256,address,uint256,uint256,uint256,uint256,bool) _auc)` | `nonpayable` | `bool` | `bool` |
...
| `userBorrowPoints(address arg0)` | `view` | `(uint256 lastPrincipal, uint256 points, uint256 lastUpdate)` | — |
| `userDebt(address arg0)` | `view` | `(uint256 amount, uint256 principal, (uint256 ltv, uint256 redemptionThreshold, uint256 liqThreshold, uint256 liqFee, uint256 borrowRate, uint256 daowry) debtTerms, uint256 lastTimestamp, bool inLiquidation)` | — |
| `userDepositPoints(address arg0, uint256 arg1, address arg2)` | `view` | `(uint256 balancePoints, uint256 lastBalance, uint256 lastUpdate)` | — |
...
- `only AuctionHouse or CreditEngine allowed`
- `only CreditEngine allowed`
