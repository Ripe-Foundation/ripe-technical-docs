...
rewards, RipeGov, HR, asset, priority-route, price-source, Underscore,
TrainingWheels, last-touch, and lite-signer domains defined by the Defaults
interface.
...
The Defaults interface has no getter for MissionControl's `userConfig` or
...
> Generated from `contracts/config/DefaultsRobinhoodLive.vy` and its tracked ABI. The ABI inventory includes inherited and exported module members and is the selector-facing reference.

### Constructor

- `constructor(address _contribTemplate)`
